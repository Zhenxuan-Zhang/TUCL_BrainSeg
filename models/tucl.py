...
# UNet part for extracting initial features
...
# A simple IntermediateSequential used for recording intermediate outputs in Transformer layers
...
class CrossAttention(nn.Module):
    def __init__(self, dim, heads=8, qkv_bias=False, qk_scale=None, dropout_rate=0.0):
        ...
        head_dim = dim // heads
        self.scale = qk_scale or head_dim ** -0.5
        # query only produces query, key_value produces key and value
...
class TUCL(TransformerTUCL):
    def __init__(self, img_dim, patch_dim, num_channels, num_classes,
        ...
        # The following decoder part is an example and may need further adjustment according to the actual task
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Load TUCL model.")
    parser.add_argument("--checkpoint", type=str, required=False, help="Path to model checkpoint")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = get_tucl().to(device)

    if args.checkpoint:
        checkpoint = torch.load(args.checkpoint, map_location=device)
        model.load_state_dict(checkpoint)
        print(f"Loaded TUCL model from {args.checkpoint}")

    model.eval()
    print("TUCL model ready for inference.")